...
public class AllNetworkQ {
    /**
     * agentList stores an ArrayList of Agent object
     */
    public ArrayList<AllAgentQ> agentsList;
     ...
     */
    public AllNetworkQ(int numAgents) {
        agentsList = new ArrayList<>();
        RLAgentList = new ArrayList<>();
        agentCount = numAgents;
        aliveAgentCount = numAgents;
        //add numAgents number of agents to the network, and activate all as Q-Learning agents
        for (int i = 0; i < numAgents; i++) {
            AllAgentQ allAgentQ = new AllAgentQ(i);
            agentsList.add(allAgentQ);
            allAgentQ.activate();
        ...
        generate2D4NSquare();//generate network topology
    ...
    public void addEdge(int a1, int a2) {
        AllAgentQ allAgentQ1 = agentsList.get(a1);
        AllAgentQ allAgentQ2 = agentsList.get(a2);
        if (allAgentQ1.getAdjLists().contains(a2) || a1 == a2) {
            return;
        }
        allAgentQ1.getAdjLists().add(a2);//add a2 to agent1's adjList
        allAgentQ2.getAdjLists().add(a1);//add a1 to agent2's adjList
    }
    ...
    /**
     * initialize the AllNetworkQ as a square 2D4N network with 10000 agents
    ...
    public void initializeQNeighborList() {
        for (int i = 0; i < agentCount; i++) {
            AllAgentQ allAgentQ = agentsList.get(i);
            for (int j = 0; j < 4; j++) {
                int neighborIndex = allAgentQ.getAdjLists().get(j);
                AllAgentQ neighbor = agentsList.get(neighborIndex);
                allAgentQ.addToQNeighborList(neighbor);
            }
        }
    }
